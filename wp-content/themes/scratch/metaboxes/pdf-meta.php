...
<!-- image -->
	...
	<div class="mypic">	
		...
		<? $wp_media_access->setGroupName('img-n'. $mb->get_the_index())->setInsertButtonLabel('Inserer')->setTab('library&post_mime_type=application/pdf'); ?>
		<!-- vignette preview -->	
		<div class="inside">
			...
			<!-- nom de l'image -->
			<p>
				<span id="pic_name"><em><? if(!empty($file_id)) { $titlepdf =  get_the_title($mb->get_the_value()); echo $titlepdf; } ?></em></span>
			</p>
			<!-- titre -->
			<? $mb->the_field('titlepdf'); ?>	
			<input id="<?php $mb->the_name(); ?>" name="<?php $mb->the_name(); ?>" type="hidden" value="<? if(!empty($file_id)) echo $titlepdf; ?>">
	...
	<p class="block">
		<a href="#" class="docopy-blocspics addpic button">Ajouter un fichier</a>
