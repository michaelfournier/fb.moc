var Blog = (function (blog) {
	...
	blog.Models.Work = Backbone.Model.extend({
        query : function (slug) {
            this.urlRoot = wp_vars.blogurl+'/api/get_post?post_type=works&custom_fields=_pmediagallery_blocspics,_pinfos_annee,_pinfos_description&include=categories,content,custom_fields,title,id&slug='+slug;
            return this;
        },
	...
	blog.Collections.WorksList = Backbone.Collection.extend({
        ...
        all : function () {
            this.url = wp_vars.blogurl+'/api/Mikictrl/get_custom_posts?post_type=works&key=_pinfos_annee&custom_fields=_pinfos_annee,_pinfos_description&order=desc&include=title,categories,gallery,custom_fields,slug&count=-1';
            return this;
        },
        query : function () {
